...
namespace sdds
{
   ...
   {
      throw std::string("Error: Copy Constructor cannot be called");
   }
   ...
   //Filles ONE item in current order that Station specifies
   ...
   void CustomerOrder::display(std::ostream& ost) const
   {
      ...
      for (size_t i = 0; i < m_cntItem; i++)
      {
         ...
         ost << setw(m_widthField) << left << m_lstItem[i]->m_itemName << " -" << right;
