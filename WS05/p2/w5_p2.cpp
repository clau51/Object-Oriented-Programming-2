...
int main(int argc, char** argv)
{
   ...
   if (argc == 5)
   {
      ...
      if (file)
      {
         while (counter < 4 && getline(file, temp))
         {
            if (temp[0] != '#')
            {
               book = temp;
   ...
   auto priceFix = [usdToCadRate, gbpToCadRate](sdds::Book& book) {
      if (book.country() == "US")
      {
         book.price() = usdToCadRate * book.price();
      }
   ...
   if (argc > 2)
   {
      ...
      if (file)
      {
         while (counter < 5 && getline(file, temp))
         {
            if (temp[0] != '#')
            {
               movie = temp;
